    ...
    try {
      ...
      if (user) {
        const currentOrderSql =
          "SELECT * FROM products_j_orders INNER JOIN orders ON products_j_orders.order_id = orders.id WHERE status = ($1) AND orders.user_id = ($2)";
        ...
        throw new Error(
          `Couldn't retrieve order. Error: No order existed with user id: ${user_id}`
        );
      }
    } catch (err) {
